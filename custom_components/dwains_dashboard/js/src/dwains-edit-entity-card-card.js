...
class DwainsEditEntityCardCard extends LitElement {
    constructor() {
      ...
      this._connectedLoadOwner = new ConnectedLoadOwner(
        (context) => this._loadEditor(context),
        {
          ...
          errorMessage: "Failed to load entity-card editor data",
        },
      );
    ...
    render() {
      ...
      if(this.mode == 'hui-card-picker'){
        return html`
          <div class="edit-element">
            <h1 style="font-size: 17px; font-weight: bold;">${translateEngine(this.hass, 'editor.select_card_for')} ${this.entity_id}</h1>
            <dwains-card-picker
...
defineDwainsElement("dwains-edit-entity-card-card", DwainsEditEntityCardCard);
